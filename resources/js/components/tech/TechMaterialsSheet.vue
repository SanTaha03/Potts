            ...
            if (m.items) {
                m.items.forEach(item => {
                    if (item.action === 'replace') {
                        ...
                        const name = item.meta?.new_device_name || 'Plante à remplacer';
                        const id = item.meta?.new_device_id || item.device_id || '???';
...
<template>
  <div class="h-full flex flex-col bg-white">
    <!-- Header -->
    <div class="pb-2 px-2 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white z-10">
        <div>
            <h2 class="text-xl font-bold text-[#1E1E1E]">Matériel nécessaire</h2>
            <p class="text-sm text-gray-500 capitalize">{{ formattedDate }}</p>
        </div>
        <button @click="$emit('close')" class="p-2 bg-gray-100 rounded-full hover:bg-gray-200 transition">
    ...
    <div class="flex-1 overflow-y-auto space-y-6 pt-2 pb-20">
        ...
        <div class="bg-[#F8F9FA] rounded-2xl p-5">
            ...
            <ul class="space-y-2 ml-1">
                <li v-for="item in checklist.base" :key="item" class="flex items-center gap-2 text-sm text-[#4C5D70]">
        ...
        <!-- TAILLE ET COUPE (If Replacement/Maintenance) -->
         <div v-if="checklist.replacement.length > 0" class="bg-[#F8F9FA] rounded-2xl p-5">
            <div class="flex items-center justify-between mb-4">
                <div class="flex items-center gap-2">
                    <Icon icon="ph:scissors-bold" class="text-lg" />
                    <h3 class="font-bold text-[#1E1E1E]">Taille et coupe</h3>
                </div>
                <div class="w-5 h-5 border-2 border-gray-400 rounded-md"></div>
            </div>
            <ul class="space-y-2 ml-1">
                <li v-for="item in checklist.replacement" :key="item" class="flex items-center gap-2 text-sm text-[#4C5D70]">
                    <span class="w-1 h-1 bg-gray-400 rounded-full"></span>
                    {{ item }}
                </li>
            </ul>
        ...
        <div v-if="checklist.specifics.length > 0" class="bg-[#F8F9FA] rounded-2xl p-5">
            <div class="flex items-center justify-between mb-4">
                <div class="flex items-center gap-2">
            ...
            <ul class="space-y-3">
                <li v-for="itemStr in checklist.specifics" :key="itemStr" class="flex items-center gap-2 text-sm text-[#1E1E1E]">
                     <span class="w-1 h-1 bg-gray-400 rounded-full shrink-0"></span>
                     <span>{{ parseItem(itemStr).name }}</span>
                     <span v-if="parseItem(itemStr).tag" class="bg-[#DFE1E3] px-1.5 py-0.5 rounded textxs font-bold text-gray-600">
