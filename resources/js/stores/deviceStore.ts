...
import {
    ...
    getDevice as getDeviceRequest,
    getDeviceHistory as getDeviceHistoryRequest,
    listDevices,
...
export const useDeviceStore = defineStore("devices", () => {
    ...
    async function fetchDevices(partial: DeviceFilters = {}, silent = false) {
        ...
        try {
            console.log("Fetching devices with filters:", filters.value);
    ...
    async function fetchDevice(id: number | string, silent = false) {
        ...
        try {
            ...
            error.value = resolveHttpErrorMessage(
                err,
                "Impossible de charger le device.",
            );
            throw err;
        } finally {
            if (!silent) loading.value = false;
    ...
    async function fetchHistory(
        id: number | string,
        sensor: "soil_pct" | "temp_c" | "light_pct",
        period: "24h" | "7d" = "24h",
    ) {
    ...
    async function createDevice(payload: DevicePayload) {
        ...
        try {
            ...
            createError.value = resolveHttpErrorMessage(
                err,
                "Impossible de créer le device.",
            );
            throw err;
        } finally {
