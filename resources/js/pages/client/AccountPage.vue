...
const goToRseReport = () => {
    router.push({ name: "rse-report" }).catch(() => undefined);
};
    ...
    <div class="bg-white font-popins p-6 pb-24">
        ...
        <div class="flex flex-col items-center mb-8">
            <div class="relative mb-4">
                <div
                    class="w-24 h-24 rounded-full overflow-hidden border-4 border-[#F7FDE7]"
                >
                    <img
                        :src="user.avatar"
                        alt="Profile"
                ...
                <button
                    class="absolute bottom-0 right-0 p-2 bg-primary-green rounded-full text-[#2F2C36] shadow-md hover:scale-105 transition"
                >
                    <Icon icon="ph:camera-bold" class="w-4 h-4" />
                </button>
            </div>
            <h2 class="text-xl font-bold text-[#2F2C36]">
                {{ user.firstName }} {{ user.lastName }}
            </h2>
            <span
                class="px-3 py-1 mt-2 text-xs font-bold text-[#2E0099] bg-[#EDE5FF] rounded-lg"
            >
                {{ user.role }}
            </span>
        </div>
        ...
        <section class="mb-6">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg font-bold text-[#2F2C36]">
        ...
        <section>
            ...
            <div class="space-y-3">
                <!-- Notification Toggle -->
                <div
                    class="flex items-center justify-between p-4 bg-white border border-gray-100 rounded-2xl shadow-sm"
                >
                    <div class="flex items-center gap-3">
                        <div class="p-2 bg-[#F2EDFF] rounded-xl text-[#9466FF]">
                            <Icon
                                icon="ph:bell-simple-ringing-bold"
                                class="w-5 h-5"
                            />
                        </div>
                ...
                >
                    <div class="flex items-center gap-3">
                        <div class="p-2 bg-[#E6F6EE] rounded-xl text-[#1B5E3F]">
                            <Icon
                    ...
                    <div class="flex items-center gap-2 text-gray-400">
                        <span class="text-xs font-medium">Annuel</span>
                ...
                <button
                    @click="handleLogout"
                    class="w-full mt-6 flex items-center justify-center gap-2 p-4 rounded-2xl border-2 border-[#FEE2E2] text-red-500 font-bold hover:bg-red-50 transition"
                >
