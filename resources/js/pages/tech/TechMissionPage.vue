...
<div class="relative min-h-screen" v-if="m">
    ...
    <div class="mt-6 px-4">
        <div class="bg-gray-200 p-1 rounded-xl flex">
            <button 
                v-for="tab in ['detail', 'client', 'notes']" :key="tab"
                class="flex-1 py-2 text-sm font-medium rounded-lg capitalize transition-all"
                :class="activeTab === tab ? 'bg-white shadow text-black' : 'text-gray-500'"
                @click="handleTab(tab as any)"
            >
                {{ tab === 'notes' ? `Notes (${m.notes?.length || 0})` : tab }}
            </button>
        </div>
    </div>
    ...
    <div v-if="activeTab === 'detail'" class="mt-6 px-6">
        <p class="text-sm text-gray-600 mb-6">
            Description de la mission... Vérification de tous les pots, arrosage si nécessaire.
        </p>

        <h3 class="font-bold text-lg mb-4">Toutes les plantes ({{ m.items?.length }})</h3>

        <div class="space-y-4">
            <template v-for="item in m.items" :key="item.id">
                
                <!-- REPLACEMENT CARD STYLE -->
                <div v-if="item.action === 'replace'" class="bg-[#F8F9FA] rounded-xl p-4 border border-gray-100">
                    <!-- Location Header -->
                    ...
                    <!-- OLD PLANT (Top) -->
                    <div class="bg-white p-3 rounded-xl shadow-sm flex items-center relative overflow-hidden">
                        <img :src="'/images/monstera.png'" class="w-12 h-12 object-cover rounded-lg bg-gray-100 grayscale opacity-70" />
                        ...
                        <!-- Visual eyes icon plant id  -->
                            <div class="relative z-20">
                                <button 
                                    @click.stop="goToPlantDetail(item.meta?.old_device_id)"
                    ...
                    <div class="flex flex-col items-center justify-center py-2 relative">
                         <div class="h-6 w-0.5 bg-gray-200 absolute top-0 bottom-0 z-0"></div>
                         <div class="bg-white rounded z-10 px-1 py-0.5 text-[10px] text-gray-400 font-bold uppercase tracking-wider flex flex-col items-center ">
                             <span>Par</span>
                    ...
                    >
                        <img :src="'/images/monstera.png'" class="w-12 h-12 object-cover rounded-lg bg-gray-100" />
                        <div class="ml-3 flex-1 z-10">
                            <div class="flex items-center gap-2">
                                <span class="font-bold text-[#1E1E1E]">{{ item.device?.name || 'Nouvelle plante' }}</span>
                                <span class="text-[10px] bg-[#EFFBD0] text-[#475F07] font-bold px-1.5 py-0.5 rounded">
                                    #{{ item.device?.device_id }}
                                </span>
                            </div>
                            <span class="text-xs text-gray-400 font-medium">Nouvelle</span>
                        </div>
                ...
                >
               <!-- Image -->
               <img :src="'/images/monstera.png'" class="w-14 h-14 object-cover rounded-xl bg-gray-200" />
               
               <div class="flex-1">
                   <div class="flex items-center gap-2 mb-1">
                      <span class="font-bold text-base text-[#1E1E1E]">{{ item.device?.name || 'Inconnue' }}</span>
                      <span class="bg-[#DFE1E3] text-gray-600 text-[10px] font-bold px-1.5 py-0.5 rounded">#{{ item.device?.device_id }}</span>
                   </div>
                   <!-- Access location from device json -->
                   <div class="text-[11px] font-medium text-[#4C5D70] flex flex-wrap gap-1 items-center">
                       <span>{{ item.device?.location?.site }}</span>
                       <Icon icon="ph:caret-right-bold" class="text-gray-400 text-[10px]" />
    ...
    <div v-if="activeTab === 'client'" class="mt-6 px-6">
        ...
        <p class="text-sm text-gray-600 mb-6">
            L'entrée se trouve côté Ouest. Code porte: 1234A.
            Ouvert de 9h à 18h.
        </p>

        <h3 class="font-bold text-lg mb-4">Référents</h3>
        
        <div class="bg-[#F8F9FA] p-3 rounded-lg mb-2 flex justify-between items-center">
             <div>
                 <div class="font-bold text-sm">Christophe Doe</div>
                 <div class="text-xs text-gray-500">Accueil • Bat A</div>
             </div>
             <div class="bg-[#EDE5FF] text-[#2E0099] px-2 py-1 rounded text-xs font-bold">
                 06 12 34 56 78
             </div>
        </div>
    </div>
    ...
    <div v-if="activeTab === 'notes'" class="mt-6 px-6">
        <div class="space-y-4 mb-20">
            <div v-for="note in m.notes" :key="note.id" class="flex flex-col gap-1">
                <div class="flex justify-between text-xs text-gray-400">
                    <span class="font-bold text-gray-800">{{ note.user.name }}</span>
                    <!-- Simple date format -->
             ...
             <p v-if="!m.notes?.length" class="text-center text-gray-400 py-10 text-sm">
        ...
        <div class="fixed bottom-32 left-6 right-6 z-10">
             <div class="bg-white p-2 rounded-xl shadow-lg border border-gray-100 flex gap-2">
                 <input 
                    v-model="noteInput"
